...
> 평소 장기 앱을 즐겨 사용하는 플레이어였지만, 개발자로서 장기의 규칙을 직접 코드로 구현하는 일은 전혀 다른 차원의 문제였습니다. 기물의 이동 경로를 계산하는 알고리즘을 구현하면서 여러 시행착오를 겪었습니다. 그 중 '차(車)'가 상대 궁성에 진입하면 서비스가 중단되는 오류가 가장 기억에 남습니다. 수를 둔 이후의 상황을 시뮬레이션하는 과정에서 발생한 문제였는데, 미리 커스텀 예외 처리를 해놓은 덕분에 오류 지점을 쉽게 찾을 수 있었습니다.
> 프리코스를 통해 메서드의 길이에 제한을 두고 indent를 줄이는 연습을 했었지만, 로직이 복잡해질수록 늘어나는 분기문(if)과 메서드 길이를 줄이기 힘들다는 것을 깨달았습니다. "한 메서드는 하나의 기능만 수행한다."는 원칙을 지키려고 했았지만, 복잡한 도메인 로직 앞에서 이를 100% 준수하기란 쉽지 않았습니다. 하지만 이렇게 고민을 했던 과정이 저에게는 좋은 개발 습관을 형성하기 위한 기반이 된 것 같습니다. 엣지 케이스를 고려하여 작성해둔 테스트 코드를 통해 리팩토링 후에 발생하는 문제에 대해 오류 지점을 빠르게 파악하여 수정할 수 있었습니다. 테스트 코드의 중요성을 느낄 수 있었습니다.
> 가장 힘들었던 부분은 시스템 설계였습니다. 기존에 Spring Boot 코드를 작성하면서는 정형화된 아키텍쳐를 따라가는 데만 익숙했지만, 이번에는 백지 상태에서 모든 클래스를 정의해야 했습니다. 초반에는 완벽한 설계를 꿈꾸며 Waterfall 방식으로 접근하려 했으나, 구현 과정에서 설계가 빈번히 수정되기도 했습니다. 이를 통해 유연한 설계의 필요성을 느꼈습니다. 특히, UI가 완성되기 전까지 로직 검증을 미루다 보니 커밋 단위를 잘게 나누지 못한 것이 가장 아쉬웠습니다. 향후 프로젝트를 진행하면서는 기능 단위의 커밋을 실천해야겠다고 다짐했습니다.
> 지난 6주간의 프리코스는 단순히 기능을 구현하는 것을 넘어, 아키텍쳐를 고민하고, 코드 가독성 및 클린 코드를 고민해봤던 귀중한 시간이었습니다. 아직 완벽하진 않지만, "협업하기 좋은 코드", "읽기 좋은 코드"를 향한 방향성을 잡았다는 점에서 성취감을 느끼고 있습니다. 6주간의 경험을 토대로, 앞으로도 끊임없이 고민하며 성장하는 개발자가 되겠습니다.
